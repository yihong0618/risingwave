...
impl NonOverlapSubLevelPicker {
    ...
    // pub fn pick_l0_multi_non_overlap_level2(
    //     &self,
    //     l0: &[Level],
    ...
    //     println!(
    //         "select_tables_len {} l0 level count {}",
    //         select_tables.len(),
    //         l0.len()
    //     );
    ...
    //         level_select_files[0].push(sst.clone());
    //         // level_select_files[0] = sst.clone();
    //         let mut overlap_info = self.overlap_strategy.create_overlap_info();
    ...
    //                 println!("target_level {} Overlapping break", target_index);

    //                 for sub_level in l0.iter() {
    //                     println!(
    //                         "sub_level id {} type {}",
    ...
    //             if pending_compact {
    //                 println!(
    //                     "target {} pending_compact overlap_files {}",
    //                     target_index,
    //                     overlap_files.len()
    //                 );
    //                 break;
    ...
    //                 println!(
    //                     "target {} overlap_files empty select_sst {:?}",
    //                     target_index,
    //                     sst.get_sst_id(),
    //                 );
    //                 continue;
    ...
    //             // level_select_files[target_index] = overlap_files[0].to_owned();
    ...
    //             println!(
    //                 "all_file_size {} min {} select_level_count {}",
    //                 all_file_size, self.min_compaction_bytes, select_level_count
    //             );
    //             continue;
    //         }

    //         // println!("level_select_files {:?}", level_select_files);
    ...
    //         score.push((all_file_size, all_file_count, level_select_files));
    //     }

    //     score.sort_by(|plan1, plan2| plan2.2.len().cmp(&plan1.2.len()));
    //     score
    // }
    ...
    //                 if overlap_files.is_empty() {
    //                     // We allow a layer in the middle without overlap, so we need to continue
    // to                     // the next layer to search for overlap
    //                     continue;
    //                 }
    ...
    //             if pending_compact {
    //                 // encountering a pending file means we don't need to continue processing
    // this                 // interval
    //                 break;
    //             }

    //             if all_file_size < self.min_compaction_bytes || select_level_count <
    // self.min_depth             {
    //                 // Avoiding too small compaction and number of layers
    //                 continue;
    //             }
    ...
    //             // computing write_amp in l0 is that we can assume that when partitions are
    // aligned,             // a file partitioned at the bottom sub_level will select the
    // smallest possible file             // write_map. When we select files that span two
    // partitions, in the expected case,             // the overlap range is larger and more
    // files are selected at the top level,             // resulting in a larger write_amp. So
    // measuring an interval by write_amp allows us             // to prefer interval-aligned
    // files.             let write_amp_delta =
    ...
    //     // 3. fewer files in the bottom sub level, containing as many smaller intervals as
    // possible.     scores
    //         .into_iter()
    //         .sorted_by(
    //             |(
    ...
    //                     // .then_with(|| write_amp_delta.cmp(write_amp_delta2)) // a way to
    // choose a                     // small guard
