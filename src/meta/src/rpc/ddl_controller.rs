...
impl DdlController {
    ...
    async fn inject_replace_table_job_for_table_sink(
    ...
    ) -> MetaResult<(u32, ReplaceTableContext, TableFragments)> {
        let table = streaming_job.table().unwrap();

        let dummy_id = match mgr {
        ...
        Ok((dummy_id, replace_table_ctx, table_fragments))
    }
    ...
    pub(crate) async fn ttt(
        ...
        if let Some(replace_table_info) = target_replace_info {
            ...
            let (dummy_id, ctx, table_fragments) = self
    ...
    async fn drop_streaming_job_v1(
        ...
        if let Some(replace_table_info) = target_replace_info {
            ...
            let result: MetaResult<()> = try {
                tracing::debug!(id = streaming_job.id(), "replacing table for dropped sink");
                let (_, context, table_fragments) = self
                    .inject_replace_table_job_for_table_sink(
                        &self.metadata_manager,
                        stream_ctx,
                        None,
...
use crate::manager::MetadataManagerV2;
...
impl DdlController {
    ...
    pub async fn drop_object(
        ...
        self.ttt(target_replace_info).await?;
