...
use risingwave_common::types::{
    ...
    OrderedF32, OrderedF64, Scalar, ScalarImpl, ScalarRefImpl, Timestamptz,
};
...
#[inline(always)]
pub fn str_to_timestamptz(elem: &str) -> Result<Timestamptz> {
    elem.parse::<DateTime<Utc>>()
        .map(|ret| Timestamptz(ret.timestamp_micros()))
...
pub fn i64_to_timestamptz(t: i64) -> Result<Timestamptz> {
    ...
    match t {
        0..E11 => Ok(Timestamptz(t * 1_000_000)), // s
        E11..E14 => Ok(Timestamptz(t * 1_000)),   // ms
        E14..E17 => Ok(Timestamptz(t)),           // us
        E17.. => Ok(Timestamptz(t / 1_000)),      // ns
...
macro_rules! for_all_cast_variants {
    ($macro:ident) => {
        $macro! {
            ...
            { timestamptz, varchar, general_to_text, false },
...
mod tests {
    ...
    fn test_timestamptz() {
        ...
        assert_eq!(timestamptz1.0, -62108094259000001);

        let mut writer = String::new();
        general_to_text(timestamptz1, &mut writer).unwrap();
        ...
        assert_eq!(timestamptz2.0, -1);

        let mut writer = String::new();
        general_to_text(timestamptz2, &mut writer).unwrap();
