...
use risingwave_common::types::{IntervalUnit, NaiveDateTimeWrapper, NaiveDateWrapper, Timestamptz};
...
pub fn tumble_start_date_time(
    ...
    let window_start = tm_diff_bin(Timestamptz(diff), window)?;
...
#[inline(always)]
pub fn tumble_start_timestamptz(time: Timestamptz, window: IntervalUnit) -> Result<Timestamptz> {
...
#[inline(always)]
fn tm_diff_bin(diff: Timestamptz, window: IntervalUnit) -> Result<Timestamptz> {
