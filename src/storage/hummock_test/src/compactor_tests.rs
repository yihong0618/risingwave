...
pub(crate) mod tests {
    ...
    use risingwave_hummock_sdk::key::{
        next_key, FullKey, TableKey, TABLE_PREFIX_LEN,
    };
    ...
    use risingwave_meta::hummock::compaction::{
        default_level_selector, CompactStatus, LevelSelector,
    ...
    use risingwave_storage::hummock::{
        ...
        SstableBuilder, SstableBuilderOptions, SstableObjectIdManager,
    ...
    impl CompactTest {
        pub fn new(throughput_multiplier: u64, test_count: u64) -> Self {
            ...
            config.split_weight_by_vnode = PARTITION_COUNT as u32;
            Self {
                group_config: CompactionGroup::new(1, config),
        ...
        async fn test_selector_compact_impl<S: SstableInfoGenerator>(&mut self, mut generator: S) {
            ...
            for i in 1..self.test_count {
                if i % CHECKPOINT_TIMES == 0 {
                    ...
                    let mut b = self.get_test_builder(
            ...
            println!("max task size: {}", self.max_task_size);
        ...
        fn pick_one_task(&mut self, start_time: u64, checkpoint: u64) -> bool {
            if let Some(task) = self.selector.pick_compaction(
                ...
                task.gc_delete_keys = (task.target_level as usize) + 1== self.handlers.len();
                task.compaction_group_id = self.group.group_id;
                task.split_by_state_table = true;
                task.split_weight_by_vnode = self.group_config.compaction_config().split_weight_by_vnode;
                task.task_id = self.global_task_id;
        ...
        async fn finish_compaction_task(&mut self, task: &mut CompactTask) {
            ...
            let opts = SstableBuilderOptions {
                ...
                block_capacity: 128,
                max_sst_size: 512 * 1024 * 1024 / self.throughput_multiplier,
                ..Default::default()
            };
            ...
            for output in ret {
                output.upload_join_handle.await.unwrap().unwrap();
        ...
        fn get_test_builder(
            ...
            let writer = self.sstable_store.clone().create_sst_writer(sst_id, writer_opts);
            SstableBuilder::new(
                sst_id,
                writer,
    ...
    impl SstableInfoGenerator for HotGenerator {
        fn generate(&mut self, kv_count: usize) -> Vec<TableKey<Vec<u8>>> {
            let mut data = Vec::with_capacity(VirtualNode::COUNT / 16 * kv_count);
            let hot_vnode_idx = (self.rng.next_u32() % 4) as usize * 2;
