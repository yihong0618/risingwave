...
use risingwave_hummock_sdk::key::{
    map_table_key_range, FullKey, TableKey, UserKey, TABLE_PREFIX_LEN,
};
...
use crate::test_utils::{gen_key, prepare_hummock_test_env, TestIngestBatch};
...
async fn test_storage_basic() {
    ...
    let mut batch3 = vec![
        ...
        (gen_key(VirtualNode::ZERO, "aa"), StorageValue::new_delete()),
    ];
    ...
    let iter = test_env
        .storage
        .iter(
            (Unbounded, Included(gen_key(VirtualNode::ZERO, "ee"))),
            epoch1,
            ReadOptions {
                table_id: TEST_TABLE_ID,
...
async fn test_state_store_sync() {
    ...
    {
        let kv_map = [
            (gen_key(VirtualNode::ZERO, "aaaa"), "1111"),
            (gen_key(VirtualNode::ZERO, "bbbb"), "2222"),
            (gen_key(VirtualNode::ZERO, "cccc"), "3333"),
            (gen_key(VirtualNode::ZERO, "dddd"), "4444"),
            (gen_key(VirtualNode::ZERO, "eeee"), "5555"),
        ];
...

    {
        let kv_map = [
            ...
            (gen_key(VirtualNode::ZERO, "cccc"), "3333"),
            (gen_key(VirtualNode::ZERO, "dddd"), "4444"),
            (gen_key(VirtualNode::ZERO, "eeee"), "6666"),
        ];
    ...
    // test iter
    {
        let iter = test_env
            .storage
            .iter(
                (Unbounded, Included(gen_key(VirtualNode::ZERO, "eeee"))),
                epoch1,
                ReadOptions {
                    table_id: TEST_TABLE_ID,
                    cache_policy: CachePolicy::Fill(CachePriority::High),
        ...
        let kv_map = [
            (gen_key(VirtualNode::ZERO, "aaaa"), "1111", epoch1),
            (gen_key(VirtualNode::ZERO, "bbbb"), "2222", epoch1),
            (gen_key(VirtualNode::ZERO, "cccc"), "3333", epoch1),
            (gen_key(VirtualNode::ZERO, "dddd"), "4444", epoch1),
            (gen_key(VirtualNode::ZERO, "eeee"), "5555", epoch1),
        ];
...
async fn test_iter_with_min_epoch() {
    ...
    let gen_key = |index: usize| -> TableKey<Bytes> {
        gen_key(VirtualNode::ZERO, format!("\0\0key_{}", index).as_str())
    };
...
async fn test_get_with_min_epoch() {
    ...
    let gen_key = |index: usize| -> TableKey<Bytes> {
        gen_key(VirtualNode::ZERO, format!("key_{}", index).as_str())
    };
