    ...
    fn bind_expr_inner(&mut self, expr: Expr) -> Result<ExprImpl> {
        match expr {
            ...
            Expr::Identifier(ident) => {
                ...
                } else if let Some(lambda_args) = self.context.lambda_args {
                    // We don't support capture, so if the expression is in the lambda context,
                    // we'll not bind it for table columns.
                    let (arg_idx, arg_type) = lambda_args.get(&ident.real_value());
                    Ok(InputRef::new(arg_idx, arg_type).into())
                } else {
            ...
            .into()),
            Expr::LambdaFunction { args, body } => self.bind_lambda_function(args, *expr),
        }
    }
