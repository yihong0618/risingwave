...
use crate::source::{
    ...
    SourceContext, SourceContextRef, SourceEncode, SourceFormat, SourceMessage, SourceMessageMeta,
    SourceMeta,
};
...
pub struct SourceStreamChunkRowWriter<'a> {
    ...
    row_meta: Option<&'a SourceMessageMeta>,
}
...
impl<'a> SourceStreamChunkRowWriter<'a> {
    ...
    fn with_meta(self, row_meta: &'a SourceMessageMeta) -> Self {
        Self {
...
async fn into_chunk_stream<P: ByteStreamSourceParser>(mut parser: P, data_stream: BoxSourceStream) {
    ...
    for batch in data_stream {
        ...
        for (i, msg) in batch.into_iter().enumerate() {
            let (msg_key, msg_payload, msg_meta) = msg.into_parts();

            if msg_key.is_none() && msg_payload.is_none() {
                tracing::debug!(
                    offset = msg_meta.offset(),
                    "got an empty message, could be a heartbeat"
                );
                parser.emit_empty_row(heartbeat_builder.row_writer().with_meta(&msg_meta));
                continue;
            }

            // calculate process_time - event_time lag
            if let SourceMeta::DebeziumCdc(source_meta) = msg_meta.source_meta() {
                let lag_ms = process_time_ms - source_meta.source_ts_ms;
                // report to promethus
                GLOBAL_SOURCE_METRICS
                    .direct_cdc_event_lag_latency
                    .with_label_values(&[&source_meta.full_table_name])
