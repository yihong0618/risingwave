...
use risingwave_common::catalog::{TableId, KAFKA_TIMESTAMP_COLUMN_NAME, TABLE_NAME_COLUMN_NAME};
use risingwave_common::types::{Datum, JsonbVal, Scalar};
...
use super::{
    SourceColumnDesc, SourceColumnType, GCS_CONNECTOR, OPENDAL_S3_CONNECTOR, POSIX_FS_CONNECTOR,
};
...
pub struct SourceMessage {
    ...
    pub meta: SourceMeta, // TODO(rc): should rename to `source_meta`
}
...
impl SourceMessage {
    /// Break down the source message into `key`, `payload`, and `meta`.
    pub(crate) fn into_parts(self) -> (Option<Vec<u8>>, Option<Vec<u8>>, SourceMessageMeta) {
        (
            self.key,
            self.payload,
            SourceMessageMeta {
...
/// The metadata of a source message.
pub struct SourceMessageMeta {
    source_meta: SourceMeta,
    split_id: SplitId,
    offset: String,
}

impl SourceMessageMeta {
    #[cfg(test)]
    pub(crate) fn for_test(source_meta: SourceMeta, split_id: SplitId, offset: String) -> Self {
        Self {
            source_meta,
            split_id,
            offset,
    ...
    /// Get the split id.
    pub(crate) fn split_id(&self) -> &str {
        &self.split_id
    }
    ...
    pub(crate) fn value_for_column(&self, desc: &SourceColumnDesc) -> Option<Datum> {
        match desc.column_type {
            ...
            SourceColumnType::Meta
                if let SourceMeta::DebeziumCdc(cdc_meta) = self.source_meta() =>
            {
                assert_eq!(
                    desc.name.as_str(),
