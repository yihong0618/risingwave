...
use crate::types::{
    ...
    NaiveTimeWrapper, OrderedF32, OrderedF64, ScalarImpl, ScalarRefImpl, Timestamptz, ToDatumRef,
};
...
fn deserialize_value(ty: &DataType, data: &mut impl Buf) -> Result<ScalarImpl> {
    Ok(match ty {
        ...
        DataType::Timestamptz => ScalarImpl::Timestamptz(deserialize_timestamptz(data)?),
...
fn deserialize_timestamptz(data: &mut impl Buf) -> Result<Timestamptz> {
