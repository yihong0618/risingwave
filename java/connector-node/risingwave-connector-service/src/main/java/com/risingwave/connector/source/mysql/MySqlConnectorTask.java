...
/**
 * The main task executing streaming from MySQL. Responsible for lifecycle management of the
 * streaming code.
 *
...
public class MySqlConnectorTask extends BaseSourceTask<MySqlPartition, MySqlOffsetContext> {
    ...
    @Override
    public ChangeEventSourceCoordinator<MySqlPartition, MySqlOffsetContext> start(
        ...
        // DBZ-3238: automatically set "useCursorFetch" to true when a snapshot fetch size other
        // than the default of -1 is given
        ...
        final EventDispatcher<MySqlPartition, TableId> dispatcher =
                new EventDispatcher<>(
                        connectorConfig,
                        topicSelector,
                        schema,
                        ...
                        new HeartbeatFactory<>(
                                ...
                                exception -> {
                                    ...
                                    switch (sqlErrorId) {
                                        case "42000":
                                            // error_er_dbaccess_denied_error, see
        ...
        final MySqlStreamingChangeEventSourceMetrics streamingMetrics =
                new MySqlStreamingChangeEventSourceMetrics(taskContext, queue, metadataProvider);

        ChangeEventSourceCoordinator<MySqlPartition, MySqlOffsetContext> coordinator =
                new DbzChangeEventSourceCoordinator<>(
                        previousOffsets,
                        errorHandler,
    ...
    private MySqlValueConverters getValueConverters(MySqlConnectorConfig configuration) {
        ...
        BigIntUnsignedHandlingMode bigIntUnsignedHandlingMode =
                BigIntUnsignedHandlingMode.parse(bigIntUnsignedHandlingModeStr);
    ...
    /**
     * Determine whether the binlog position as set on the {@link MySqlOffsetContext} is available
     * in the server.
     *
    ...
    protected boolean isBinlogAvailable(MySqlConnectorConfig config, MySqlOffsetContext offset) {
        ...
        if (gtidStr != null) {
            ...
            // GTIDs are enabled, and we used them previously, but retain only those GTID ranges for
            // the allowed source UUIDs ...
            ...
            if (gtidSet.isContainedWithin(availableGtidSet)) {
                LOGGER.info(
                        ...
                        availableGtidSet,
                        gtidSet);
                final GtidSet knownServerSet =
                        availableGtidSet.retainAll(config.gtidSourceFilter());
    ...
    private boolean validateSnapshotFeasibility(
            MySqlConnectorConfig config, MySqlOffsetContext offset) {
        if (offset != null) {
            ...
            if (!config.getSnapshotMode().shouldSnapshot()) {
                // Look to see what the first available binlog file is called, and whether it looks
                // like binlog files have
                ...
                if (earliestBinlogFilename == null) {
                    LOGGER.warn(
                            "No binlog appears to be available. Ensure that the MySQL row-level binlog is enabled.");
                } else if (!earliestBinlogFilename.endsWith("00001")) {
                    LOGGER.warn(
                            "It is possible the server has purged some binlogs. If this is the case, then using snapshot mode may be required.");
                }
            }
        }
        return false;
