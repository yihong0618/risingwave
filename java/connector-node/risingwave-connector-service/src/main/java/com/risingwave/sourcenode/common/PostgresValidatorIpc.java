...
public class PostgresValidatorIpc implements AutoCloseable {
    static final Logger LOG = LoggerFactory.getLogger(PostgresValidatorIpc.class);
    ...
    public void validateTableSchema() throws Exception {
        ...
        try (var stmt = conn.prepareStatement(sqlStmts.getProperty("postgres.table_schema"))) {
            ...
            while (res.next()) {
                ...
                if (!isPostgresDataTypeCompatible(dataType, srcCol.dataType)) {
                    throw new RuntimeException(
                            String.format("incompatible data type of column %s", srcCol.name));
                }
            }
        }
    }
            ...
            try (var stmt =
                ...
                while (res.next()) {
                    ...
                    for (int i = 0; i < tableSchema.columns.size(); i++) {
                        String columnName = tableSchema.columns.get(i).name;
                        ...
                        if (i == tableSchema.columns.size() - 1) {
            ...
            if (!isTableOwner && null != tableOwner) {
                try (var stmt =
                    ...
                    while (res.next()) {
                        ...
                        if (Arrays.asList(users).contains(props.get(DbzConnectorConfig.USER))) {
                            isTableOwner = true;
                            break;
                        }
                    }
    ...
    private boolean isPkMatch(TableSchema sourceSchema, Set<String> pkFields) {
