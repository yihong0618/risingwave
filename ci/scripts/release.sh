...
dnf install -y perl-core wget python3.11 python3.11-devel cyrus-sasl-devel rsync
...
cargo build -p risingwave_cmd --bin risectl --features "rw-static-link" --features all-udf --profile release
